...
bool Board::move(VID_T vid, int amount)
{
    if(isLegalMove(vid, amount)) {
        ...
        updateBoard();
        reverseMoveStack.push(std::make_pair(vid, amount*-1));
        return true;
    }
    else {
        return false;
...
{
    if(reverseMoveStack.empty())
    {
        throw std::underflow_error("No move made");
    }
    else
    {
        this->move(reverseMoveStack.top().first, reverseMoveStack.top().second);
        reverseMoveStack.pop();
        reverseMoveStack.pop();
    }
}
...
Board::MovePairList Board::potentialMoves() const
{
    ...
    MovePairList possibleMovesList;
    VehicleMap::const_iterator it;
    for(it = vehicles_.begin(); it != vehicles_.end(); it++)
    {
        int i = 1;
        while(isLegalMove(it->first, i))
        {
            possibleMovesList.push_back(std::make_pair(it->first, i));
            i++;
        }
        int j = -1;
        while(isLegalMove(it->first, j))
        {
            possibleMovesList.push_back(std::make_pair(it->first, j));
            j--;
        }
    }
...
{
    // To avoid compile errors in skeletong - Replace this
    std::string thisBoard;
    std::string thatBoard;
    // Convert to String than Use < than operators for strings
    for(unsigned int i = 0; i < this->grid_.size(); i++)
    {
        for(unsigned int j = 0; j < this->grid_.at(i).size(); j++)
        {
            thisBoard += this->grid_.at(i).at(j);
        }
    }
    for(unsigned int i = 0; i < other.grid_.size(); i++)
    {
        for(unsigned int j = 0; j < other.grid_.at(i).size(); j++)
        {
            thatBoard += other.grid_.at(i).at(j);
        }
    }
